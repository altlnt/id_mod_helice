...
class Log():
    ...
    drone_type="plane",is_sitl=True,roll_gain=1.0,pitch_gain=1.0):
    ...
    def import_log(self,tcut=((-1.0,0.0),(1e15,1e100))):
        ...
        if 'DATA.csv' in os.listdir(dpath):
            print("FOUND CSV FILE, LOADING")
            self.DATA=pd.read_csv(os.path.join(dpath,"DATA.csv"))
        ...
        if 'tcut.csv' in os.listdir(dpath):
        ...
        DATA.to_csv(os.path.join(self.ulg_path,"DATA.csv"))
        self.DATA=DATA
        return
...
log='log_147_2019-3-20-09-43-54'
