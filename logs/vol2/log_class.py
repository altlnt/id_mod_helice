...
class Log():
    ...
    def import_log(self):
        ...
        att=asarray([euler.mat2euler(quaternions.quat2mat(attitude.values[i,4:4+4])) for i in range(len(attitude))]).reshape(-1,3)
        R=array([quaternions.quat2mat(attitude.values[i,4:4+4]) for i in range(len(attitude))]).reshape(-1,9)
