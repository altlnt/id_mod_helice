...
Q=1.0*np.eye(len(id_variables)+3)*(tdf['dt'].mean())**2
        
Q[:3,:3]=2.0*np.eye(3)
# Q[-2:,-2:]=1e-2*np.eye(2)

init_P=10.0*np.eye(len(id_variables)+3)
...
#     Q=np.diag(X[:len(id_variables)])
#     P_init={}
#     for j,k in enumerate(id_variables.keys()):
#         P_init[k]=X[len(id_variables)+j]
        
#     x,p,y=run(tdf,id_variables,P_init,Q=Q,R=1e-2)
#     c=(tdf['acc_ned_grad[0]']-y[:,0])**2
#     c+=(tdf['acc_ned_grad[1]']-y[:,1])**2
#     c+=(tdf['acc_ned_grad[2]']-y[:,2])**2
#     c=np.mean(c)
#     print(c)
#     return c
    
# X0=[1e-5]*2*len(id_variables)
# from scipy.optimize import minimize
# sol=minimize(find_best_params,X0,method="SLSQP")
