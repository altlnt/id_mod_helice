...
def main_func(x):
   ...
   # log_path="./logs/vol12/log_real.csv"
    ...
    def arg_wrapping(batch,id_variables,scalers,data_index,speed_pred_previous,omegas_pred,scipy_calls=False):
        i=data_index
        ...
        if id_wind and not scipy_calls:
    ...
    def pred_on_batch(batch,id_variables,scalers,scipy_calls=False):
        ...
        for i in batch.index:
            ...
            X=arg_wrapping(batch,id_variables,scalers,i,speed_pred_prev,omegas_pred,scipy_calls=scipy_calls)
    ...
    def fun_cost_scipy(X,batch,scalers,writtargs):
        ...
        acc_pred,speed_pred,omegas,square_error_a,square_error_v,jac_error_a,jac_error_v=pred_on_batch(batch,id_var,scalers,scipy_calls=True)
    ...
    def train_loop(data_batches,id_var,n_epochs=n_epochs):
        ...
        for n in range(n_epochs):
                ...
                if fit_strategy=="custom_gradient":
                    ...
                    if wind_signal:
                        for key_ in ('vw_i','vw_j'):
                            ...
                            tparr[batch_['index']]=new_id_variables[key_]*np.ones(len(batch_))
                    ...
                    if ns!=-1 or k%100 :
                        ...
                        if wind_signal:
                            ...
                            nsave="WINDSIG_epoch_%i_batch_%i"%(n,k)
                            windsave_df.to_csv(os.path.join(spath,nsave+".csv"))
            ...
            if wind_signal:
                windsave_df=pd.DataFrame(data=[id_variables['vw_i'],id_variables['vw_j']],columns=['w_i','w_j'])
                nsave="WINDSIG_epoch_%i"%(n)
                windsave_df.to_csv(os.path.join(spath,nsave,".csv"))
