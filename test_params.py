...
def test_params(batch,id_variables):  
        
    acc_pred,speed_pred,omegas,square_error_a,square_error_v,jac_error_a,jac_error_v=pred_on_batch(batch,id_variables,scalers)
    print("SQERRR :")
    print("V || ",np.mean(square_error_v,axis=0))
    print("A || ",np.mean(square_error_a,axis=0))
    fig=plt.figure()
    ax_ai=fig.add_subplot(3,2,1)
    ax_aj=fig.add_subplot(3,2,3)
    ax_ak=fig.add_subplot(3,2,5)
    ax_vi=fig.add_subplot(3,2,2)
    ax_vj=fig.add_subplot(3,2,4)
    ax_vk=fig.add_subplot(3,2,6)

    
    vlog_i,vlog_j,vlog_k=batch['speed[0]'],batch['speed[1]'],batch['speed[2]']
    alog_i,alog_j,alog_k=batch['acc_ned_grad[0]'],batch['acc_ned_grad[1]'],batch['acc_ned_grad[2]']
        
    vpred_i,vpred_j,vpred_k=speed_pred[:,0].T,speed_pred[:,1].T,speed_pred[:,2].T
    apred_i,apred_j,apred_k=acc_pred[:,0].T,acc_pred[:,1].T,acc_pred[:,2].T

    ax_ai.plot(batch['t'],alog_i,label="acc i")
    ax_ai.plot(batch['t'],apred_i,label="acc i p")
    ax_ai.grid(),ax_ai.legend()
    
    ax_aj.plot(batch['t'],alog_j,label="acc j")
    ax_aj.plot(batch['t'],apred_j,label="acc j p")
    ax_aj.grid(),ax_aj.legend()
    
    ax_ak.plot(batch['t'],alog_k,label="acc k")
    ax_ak.plot(batch['t'],apred_k,label="acc k p")
    ax_ak.grid(),ax_ak.legend()

    ax_vi.plot(batch['t'],vlog_i,label="v i")
    ax_vi.plot(batch['t'],vpred_i,label="v i p")
    ax_vi.grid(),ax_vi.legend()
    
    ax_vj.plot(batch['t'],vlog_j,label="v j")
    ax_vj.plot(batch['t'],vpred_j,label="v j p")
    ax_vj.grid(),ax_vj.legend()

    ax_vk.plot(batch['t'],vlog_k,label="v k")
    ax_vk.plot(batch['t'],vpred_k,label="v k p")
    ax_vk.grid(),ax_vk.legend() 
       
    plt.figure()
    
    for i in range(6):
        
        plt.gcf().add_subplot(3,2,1+i)
        omega_c=batch['omega_c[%i]'%(i+1)][:] 
        omega=omegas[:,i].T
        plt.gca().plot(batch['t'],omega_c,label="c")
        plt.gca().plot(batch['t'],omega,label="p")
        plt.gca().legend(),plt.gca().grid()
